...
- Добавлена возможность переключения между языками (русский/английский);
- Настроена синхронизация проекта с сервисом [https://gitlocalize.com/repo/4645](https://gitlocalize.com/repo/4645);
- Реализована динамическая подгрузка языка.

## Запуск
1. Поднять сервер - [инструкция](server/README.md) 
2. Поднять клиент - [инструкция](client/README.md)
3. Поднять билд-сервер - [инструкция](build-server/README.md)
3. Поднять билд-агент - [инструкция](build-agent/README.md)

## Ответы на вопросы
**_Как приложение узнает, какой язык выбран у пользователя?_**

С помощью пакета `i18next-browser-languagedetector`. В ядре (под капотом)
вызывается `navigator.language` свойство, которое возвращает локаль, например `ru/en`.

**_Какие типы контента поддерживают переключение языка и как вы это реализовали_**

Переключение языка поддерживают текст, даты и числа. Для локализации, плюрализацию и дат
использовал пакеты `i18next`, `react-i18next`. 

**_Как переводы попадают на клиент? как изменилась сборка приложения?_**

Пакет `i18next-http-backend` скачивает всегда английский (язык по умолчанию, используется
в случае отсутствия перевода на других языках) и, по необходимости, русский язык.
Т.е русский язык на клиент поподает не сразу, а только в момент выбора. Сборка приложения
не изменилась.

**Из особенностей реализации**

`GitLocalize` не поддерживает плюрализацию, некоторые поля пришлось перевести не через сервис.
Из-за ручного перевода возник рассинхрон, в интерфейсе написано, что некоторые
файлы переведены не полностью.

Еще `GitLocalize` не умеет работать со спец. символами, фраза `<0>Synchronize every</0> <1>{{count}}</1> <2/> <3>minute</3>`
превращается в `&lt;0&gt;Synchronize every0&gt; &lt;1&gt;{{count}}1&gt; &lt;2/&gt; &lt;3&gt;minute3&gt;`
и в таком виде приходит на клиент. Тоже перевел вручную.

# Cервис-воркер

- Реализован сервис-воркер для кэширования статики;
- Добавлена возможность получения push-уведомлений в браузере.
...
- Запустить тесты `npm sw-test`.

## Стратегия
Выбрана стратегия `cache or update`. Стратегия решает проблему актуальности
данных. Контент обновляется после следующей перезагрузки страницы. 

CRA, каждому статичному файлу, генерирует уникальный префикс,
указать явно какие файлы кэшировать не получилось.

Поэтому статика кэшируется в фоне, при очередном запросе.
...
2. Запустить тесты `npm run e2e-test`.
